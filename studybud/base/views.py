...
from .models import Message, Room, Topic, Message, Reply, User
...
def home(request):
    ...
    ) # to pass to home.html

    topics = Topic.objects.all()[0:5] # to pass to home.html
    room_count = rooms.count() # to pass to home.html
    room_messages = Message.objects.filter(Q(room__topic__name__icontains=q)) # to pass to home.html
...
def userProfile(request, pk):
    ...
    topics = Topic.objects.all() # to pass to home.html
...
def updateRoom(request, pk):
    room = Room.objects.get(id=pk)
    form = RoomForm(instance=room) # adding instance means the forms will be prefilled
    topics = Topic.objects.all()
...
def activityPage(request):
    room_messages = Message.objects.all() # to pass to home.html
